...
const STATUS_STYLE = {
  ...
  en_attente: { label: 'En attente', color: '#8B6914', bg: 'rgba(139,105,20,0.1)'  },
    ...
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
          <div style={{ fontFamily: DM, fontSize: 8, color: '#8B6914', letterSpacing: '2px', opacity: 0.5 }}>Projets</div>
          <div style={{ fontFamily: DM, fontSize: 9, color: '#B0A898' }}>{projetsEnCours}</div>
        </div>
        {urgentProjects.length === 0 ? (
          <p style={{ fontFamily: JO, fontSize: 11, color: '#B0A898', textAlign: 'center', padding: '0.5rem 0' }}>Aucun projet urgent</p>
          ...
          return (
            <div key={i} style={{ background: '#F8F5F0', border: '0.5px solid #E8E4DC', borderRadius: 2, padding: '0.55rem 0.75rem', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: JO, fontSize: 12, fontWeight: 500, color: '#3A3630', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.client_name}</div>
                <div style={{ fontFamily: JO, fontSize: 10, color: '#A8A098', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.project_type}</div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2, flexShrink: 0 }}>
                <span style={{ fontFamily: DM, fontSize: 8, fontWeight: 600, color: s.color, background: s.bg, padding: '2px 5px', borderRadius: 2 }}>{s.label}</span>
                {days !== null && (
                  <span style={{ fontFamily: DM, fontSize: 8, color: days <= 2 ? TERRA : '#B0A898' }}>
        ...
        {members.map((m, i) => (
          <div key={i} style={{ background: '#F8F5F0', border: '0.5px solid #E8E4DC', borderRadius: 2, padding: '0.65rem 0.75rem', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontFamily: JO, fontSize: 10, color: '#A8A098', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.role}</div>
            </div>
            <div style={{ width: 5, height: 5, borderRadius: '50%', background: DOT_COLORS[i % DOT_COLORS.length], flexShrink: 0 }} />
          </div>
        ))}
        {members.length === 0 && (
          <p style={{ fontFamily: JO, fontSize: 11, color: '#B0A898', textAlign: 'center', padding: '0.5rem 0' }}>Aucun membre</p>
        )}
      </div>
